...
int16_t requested_throttle(){
	static uint8_t smoothing_flag = SMOOTHING_STATE_NONE;
	static uint32_t smoothing_start_tick = 0;

    uint32_t max_power = get_max_power();

    if (smoothing_flag == SMOOTHING_STATE_DOWN) { // if we are in the smoothing down sequence
		// go from 60kW max power to 50kW max power over the course of a few seconds
		...
		if (interval_ms > SMOOTHING_DOWN_TIME_MS) { // if we have spend enough time smoothing down, can start smoothing up
			...
			max_power = max_power - ((float)interval_ms / (float)SMOOTHING_DOWN_TIME_MS) * SMOOTHING_POWER_DELTA_W;
		}
	} else if (smoothing_flag == SMOOTHING_STATE_UP) { // else, we are in the smoothing up sequence
		// go from 50kW max power back up to 60kW max power
