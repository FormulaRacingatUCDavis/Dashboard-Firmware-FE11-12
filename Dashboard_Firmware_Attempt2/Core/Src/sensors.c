...
// Usage: rpm_to_torque_table[rpm - 3474] => torque at that rpm
// rpm starts affecting torque limit at 3474 rpm, for lower rpms can simply use 220 Nm (default max)
// max rpm that can be used for indexing is 6049 rpm
...
//	if (brake.percent >= BRAKE_BSPD_THRESHOLD) return 0;
	...
	if (get_accumulator_power() >= MAX_POWER_ACCUMULATOR_W) {// if exceed power limit of 70kW, severely limit torque
		if (max_torque > 120) {
			...
			max_torque = 120; // why max torque is not set to 0: would lead to lurching as power oscillates above and below 70kW
		}
	} else if (get_accumulator_power() >= SMOOTHING_POWER_THRESHOLD_W) { // start smoothing torque if getting close to 70kW
		if (motor_speed < 6049) { // use table to limit torque
			uint8_t max_torque_rpm = rpm_to_torque_table[motor_speed - 3474];
    ...
    if (throttle1.percent < DEADZONE_PERCENTAGE && car_speed_kph > 3.106 && soc < 95) {
